...
void App_Init(void)
{
    ...
    /*
     * 안전을 위해 움직이면 안 되는 ID는 Torque OFF
     * 움직일 ID만 Torque ON
     */
    for (int i = 0; i < sizeof(dxl_ids) / sizeof(dxl_ids[0]); i++)
    {
        ...
        if (IsMovingID(id))
        {
            DXL_Write1Byte(id, DXL_ADDR_TORQUE_ENABLE, 1);
            HAL_Delay(20);
        }
        else
        {
            /*
             * 0,3,6,9,12,15는 절대 움직이면 안 되므로 Torque OFF
             * 이건 Goal Position이 아니라 토크 비활성화 명령임
             */
            DXL_Write1Byte(id, DXL_ADDR_TORQUE_ENABLE, 0);
            HAL_Delay(20);
        }
    }
}
...
void App_Loop(void)
{
    ...
    for (int i = 0; i < sizeof(dxl_ids) / sizeof(dxl_ids[0]); i++)
    {
        ...
        {
            /*
             * 0,3,6,9,12,15는 여기서 절대 Goal Position 안 보냄.
             * 필요하면 현재 위치만 읽음.
             */
            uint16_t present = 0;
            dxl_last_comm_ok = DXL_Read2Byte(id, DXL_ADDR_PRESENT_POSITION, &present);
