...
static uint32_t prev_imu_valid_frame_count = 0;
static float filtered_roll_cmd = 0.0f;
static float filtered_pitch_cmd = 0.0f;

static float Balance_ApplyDeadband(float value);
static float Balance_LimitStep(float target, float current, float max_step);
...
uint8_t Balance_CalibrateIMU(uint16_t sample_count, uint32_t sample_interval_ms, uint32_t timeout_ms)
{
    ...
    uint32_t last_sample_frame_count = imu_valid_frame_count;
    ...
    while (samples < sample_count)
    {
        ...
        if (!imu_parse_ok || imu_valid_frame_count == last_sample_frame_count)
        {
            continue;
        }
        ...
        last_sample_frame_count = imu_valid_frame_count;
        roll_sum += imu_roll;
        pitch_sum += imu_pitch;
        yaw_sum += imu_yaw;
        samples++;
    ...
    prev_imu_valid_frame_count = imu_valid_frame_count;
    ...
    filtered_roll_cmd = 0.0f;
    filtered_pitch_cmd = 0.0f;
...
uint8_t Balance_Update(Balance_Output *out)
{
    ...
    if (!balance_calibrated || !imu_parse_ok ||
        imu_valid_frame_count == prev_imu_valid_frame_count)
    {
        return 0;
    }
    ...
    balance_roll_error = Balance_ApplyDeadband(balance_roll_error);
    balance_pitch_error = Balance_ApplyDeadband(balance_pitch_error);

    raw_roll_cmd = (BALANCE_KP_ROLL * balance_roll_error) +
                   (BALANCE_KD_ROLL * balance_roll_rate);
    raw_pitch_cmd = (BALANCE_KP_PITCH * balance_pitch_error) +
                    (BALANCE_KD_PITCH * balance_pitch_rate);

    raw_roll_cmd = Balance_LimitStep(raw_roll_cmd, filtered_roll_cmd, BALANCE_MAX_STEP_CMD);
    raw_pitch_cmd = Balance_LimitStep(raw_pitch_cmd, filtered_pitch_cmd, BALANCE_MAX_STEP_CMD);

    filtered_roll_cmd += (raw_roll_cmd - filtered_roll_cmd) * BALANCE_FILTER_ALPHA;
    filtered_pitch_cmd += (raw_pitch_cmd - filtered_pitch_cmd) * BALANCE_FILTER_ALPHA;

    balance_roll_cmd = filtered_roll_cmd;
    balance_pitch_cmd = filtered_pitch_cmd;
...
static float Balance_ApplyDeadband(float value)
{
    if (value > -BALANCE_DEADBAND_DEG && value < BALANCE_DEADBAND_DEG)
    {
        return 0.0f;
    }
