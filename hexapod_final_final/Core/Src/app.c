...
#define APP_WALK_STEP_INTERVAL_MS 500
#define APP_WALK_FEMUR_TICK       45
#define APP_WALK_TIBIA_TICK       35
...
#if APP_JOINT_DIRECTION_TEST_MODE || APP_STARTUP_WAVE_MODE || APP_AIR_WALK_MODE
...
#endif
#if APP_AIR_WALK_MODE
volatile uint8_t air_walk_phase = 0;
volatile uint32_t air_walk_step_count = 0;
#endif
...
#endif
#if APP_AIR_WALK_MODE
static void App_RunAirWalk(void);
static void App_SendAirWalkGroup(uint8_t group, uint8_t lifted);
static void App_SendAirWalkLeg(uint8_t leg, int16_t femur_offset, int16_t tibia_offset);
#endif
...
void App_Init(void)
{
    ...
    DXL_TorqueEnableAll();
#elif APP_STARTUP_WAVE_MODE || APP_AIR_WALK_MODE
    DXL_TorqueEnableAll();
    DXL_MoveHomePosition();
...
void App_Loop(void)
{
#if APP_SEND_GOAL_MODE && !APP_IMU_TEST_MODE && !APP_DXL_TEST_MODE && !APP_JOINT_DIRECTION_TEST_MODE && !APP_AIR_WALK_MODE
...
#endif
#if (!APP_DXL_TEST_MODE && !APP_JOINT_DIRECTION_TEST_MODE && !APP_AIR_WALK_MODE) || APP_IMU_TEST_MODE
...
static void App_RunAirWalk(void)
{
    ...
    if ((now_ms - last_step_ms) < APP_WALK_STEP_INTERVAL_MS)
    {
        return;
    }
    ...
    switch (air_walk_phase)
    {
    case 0:
        App_SendAirWalkGroup(0, 1);
        air_walk_phase = 1;
        break;
    case 1:
        App_SendAirWalkGroup(0, 0);
        air_walk_phase = 2;
        break;
    case 2:
        App_SendAirWalkGroup(1, 1);
        air_walk_phase = 3;
        break;
    default:
        App_SendAirWalkGroup(1, 0);
        air_walk_phase = 0;
        break;
    }
...
static void App_SendAirWalkGroup(uint8_t group, uint8_t lifted)
{
    static const uint8_t tripod_group[2][3] = {
        {0, 2, 4},
        {1, 3, 5}
    };
    int16_t femur_offset = lifted ? APP_WALK_FEMUR_TICK : 0;
    int16_t tibia_offset = lifted ? APP_WALK_TIBIA_TICK : 0;

    for (uint8_t index = 0; index < 3; index++)
    {
        App_SendAirWalkLeg(tripod_group[group][index], femur_offset, tibia_offset);
        HAL_Delay(8);
    }
}
...
static void App_SendAirWalkLeg(uint8_t leg, int16_t femur_offset, int16_t tibia_offset)
{
    uint8_t femur_id = (uint8_t)((leg * 3U) + 1U);
    uint8_t tibia_id = (uint8_t)((leg * 3U) + 2U);
    uint16_t femur_position = DXL_ClampPosition((int32_t)app_home_position[femur_id] + femur_offset);
    uint16_t tibia_position = DXL_ClampPosition((int32_t)app_home_position[tibia_id] + tibia_offset);

    (void)DXL_WriteGoalPosition(femur_id, femur_position);
    HAL_Delay(4);
    (void)DXL_WriteGoalPosition(tibia_id, tibia_position);
}
#endif
#if APP_JOINT_DIRECTION_TEST_MODE
static void App_RunJointDirectionTest(void)
